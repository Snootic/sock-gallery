...
import { useMeshes } from './meshesContext'
...
export function useCollision() {
  ...
  return (mesh: Mesh) => {
    ...
    for (let i = 0; i < positionAttribute.count; i++) {
      ...
      const collisionResults = ray.intersectObjects(meshesRef.current.filter((m) => m !== mesh));
      if (collisionResults.length > 0 &&collisionResults[0].distance < directionVector.length()) {
        if (collisionResults[0].face && collisionResults[0].face.normal) {
          ...
          worldNormal.transformDirection((collisionResults[0].object as Object3D).matrixWorld);
